...
{
	type => "rule",
	comment => "json parser - issue #1576",
	conf => qq(
		SecRuleEngine On
		SecRequestBodyAccess On
		SecDebugLog $ENV{DEBUG_LOG}
	...
	match_log => {
		error => [ qr/ModSecurity: Warning. Pattern match "baoeuar" at ARGS:foo.|ModSecurity: JSON support was not enabled/s, 1 ],
		debug => [ qr/Adding JSON argument 'foaoeuaoeuo' with value 'bar'|JSON support was not enabled/, 1 ],
	},
	match_response => {
		status => qr/^200$/,
	},
